...
As I'd already defined the games by game numbers and dates, the most obvious way to create a grouping that would print easily was to use the, which is called `week_num` in `datetime`. This would allow me to see what games were occurring that week, as several games would have the same `week_num`. 
