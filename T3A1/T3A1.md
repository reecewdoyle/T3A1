...
When the app is activated the first thing it does is look for a file called `results.csv`. If it exists, it opens the file in `read mode`, invoked by `results = open(file`
```and then closes it. 

If the file doesn't exist, it creates the filed with the 
***
### References
