...
Each member will `fork` or `clone` the Repo that was established to work on using their local machine. Once a feature or task is completed, it is pushed up to the Repo

![git-workflow](git-workflow.png)

Each team member could be working on different branches and at different stages of the process at a time. Therefore, it is important that everything is always pushed back to the Repo.
