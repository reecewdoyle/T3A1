...
It's crucial that detailed comments are included with each commit. If it is a fix relating to a ticket that has been generated in GitHub, it is wise to include the ticket number in the commit comment.
...
The Team Leader has the opportunity within the GitHub website's framework to offer feedback and comments. 

#### Testing

Part of the review process will also including testing, as per Test Driven Design principles. This could be tests that are specifically designed and automated, or manual testing from the Team Leader. 

Once the Team Leader is satisfied, the code will be merged into the codebase. 

#### Release Management

A Release branch will be created to prepare the product for release. This is where the final level of testing, last minute fixes, and Version Tagging can happen (`v1.0.0`). Once all of this has cleared, the Release can be merged into the Main Branch.

#### Hotfix Management

Any last minute fixes that are discovered in the release phase can be taken care of on a dedicated Hotfixes branch. If the development process has been effective, there should be no major issues in this stage. Once the issue has been fixed it can be tagged (`v1.0.1`) and merged back to the main branch. 

#### Monitoring and Maintenance

After release it becomes a continuous improvement process where the team monitors for issues, fixes them as they come up, and develops new features. 
