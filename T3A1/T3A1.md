...
You can check if a particular property is present in an object with the `hasOwnProperty()` method. You could also just check if the property is `undefined`. 
```javascript
const person = {
    firstname: "Bruce",
    surname: "Wayne",
...
// Output:  by just using `in` instead of `hasOwnProperty()` it returns undefined
