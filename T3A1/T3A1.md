...
```javascript
	model = models[randomIntFromInterval(0,makes.length-1)] 
/* 
`makes.length-1` is making reference to the length of the `makes` array, which only contains 3 elements. This means there are only 3 possible answers, meaning that the only 3 options you'll see are a `model` value of `1980`, `1981`, or `1982`. 
*/
