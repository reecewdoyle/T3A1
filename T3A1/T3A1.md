...
Parsing is the process of analysing a JSON string and creating an object that can be manipulated and accessed through your programming language; in this case, JavaScript. When you receive JSON strings like this from an API, they need to be parsed so they can be a useable JSON object.
...
The inverse of the above process is when we've already got a JSON object and want to turn it into a JSON string for storing on a web server. 

```javascript
const person = {name: "Tony Stark", alias: "Iron Man", }
```
