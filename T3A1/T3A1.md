...
The first kind of loop to look at is a `for` loop. 
...
*/
    ...
    sum += numbers[i];
// `sum` will store the value of the `numbers` at a given interval. It will add the value to the total value of `sum` after each iteration. In this example, on the 6th ireration, i = 5, therefore, i < numbers.length 
}
