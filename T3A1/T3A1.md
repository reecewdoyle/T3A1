...
If I wanted to remove everything after index 3 in my pokemon array, it would look like this:
```javascript
let pokemon = [`Onyx`, `Spearow`, `Bulbasaur`, `Charmander`, `Squirtle`, `Pikachu`];
pokemon.splice(2);
// => [`Squirtle`, `Pikachu`]
...
But if I wanted to remove a range of elements from the array, it woul
