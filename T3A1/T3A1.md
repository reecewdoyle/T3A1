...
I think if I were doing this project again, I would do either just the `Musicians` or just the `Setlists` components on their own. Each of these problems would have met the scope of the requirements of the assignment. 

I think I would've favoured the `Setlist` idea, as I would be able to take advantage of using the Spotify API, which allows you to populate the table with data about each song, specifically the `Name, Title, Key, Genre, BPM` etc. 
