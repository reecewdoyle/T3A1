...
First I had to establish the variable of `users_choice` having a value of `""`, which is essentially nothing. This was so we could call it in the `while` loop. 
 ...
 decided that `8` would be what I used to exit the menu, so I used the logic `while users_choice != "8":` to mean "while the users choice isn't 8". A selection of `8` would send you into the `elif` block of `    elif (users_choice == "8"): continue`. The `continue` would send you into a print function 
***
### References
