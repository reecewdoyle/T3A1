...
`new` creates a new instance of the `Model` class from above. `make` and `model` are passed as arguments. This allows access to the code above that randomly selects a `make` from the array of ["Ford", "Holden", "Toyota"], and a 
*/
	console.log(mycar.show()) 
