...
Like all programming languages, JavaScript has it's own built in data structures. They might not be exactly the same across all languages. Python differentiates between a floating-point number or `float` and an integer or `int`, where as JavaScript has a simpler number system and just uses `Numbers` and `BigInt` when they reach the extremities of how large a number JS can handle. But they both use a numeric data type. They also use a strings, booleans and objets. 
