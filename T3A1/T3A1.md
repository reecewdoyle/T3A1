...
// Iterate over the properties of the person object for (let key in person) { // Check if the property value is a string if (typeof person[key] === 'string') { // Transform the property value to uppercase person[key] = person[key].toUpperCase(); } }
```
