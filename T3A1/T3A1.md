...
The Team Leader will delegate tasks for each member. 

Each member will `fork` or `clone` the Repo that was established to work on using their local machine. Once a feature or task is completed, it is pushed up to the Repo (which in the image below is referred to as `origin`). This is crucial, as each team member could be working on different branches and at different stages of the process at any given time.

It's crucial that detailed comments are included with each commit. If it is a fix relating to a ticket that has been generated in GitHub, it is probably wise to include the number in the commit comment. 

conventionalcommits.org suggest a framework where the reason for each commit is clearly stated with the comment:
* fix: a commit type of `fix` is designed to patch issues within the codebase.
* feat: a commit type of `feat` that adds a new feature to the codebase. 
* `BREAKING CHANGE` or `!` within the commit type (`fix!`) implies a fix that will involve a change to an A
