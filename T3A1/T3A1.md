...
// This is a variable that is created to hold the value of the pokemon from the array that gets `shifted` by the `eliteFourBattleTeam.pop()` method.
...
A `do-while` loop is similar to a while loop, but the `do` portion guarantees that the loop runs at least once. This is 
```javascript
const eliteFourBattleTeam = ['Venusaur', 'Charizard', 'Blastoise', 'Alakazam', 'Gengar', 'Gyarados'];

let fightingPokemon; // Declare the variable outside the loop scope

do {
    fightingPokemon = eliteFourBattleTeam.shift(); // Shift the first Pokémon from the array
    console.log(fightingPokemon + ", I choose you!"); // Output the chosen Pokémon
} while (eliteFourBattleTeam.length > 0); // Continue the loop as long as there are Pokémon left in the array
