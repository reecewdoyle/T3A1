...
`Symbol` is a newer data type in JavaScript. They create a completely unique identifier, even if they look the same. They can be used for hiding information. 
```javascript
...
// even though we've passed `cat` as an argument for both instances of a Symbol, they're not equal. This is because of the unique identifier that isn't displayed. 
