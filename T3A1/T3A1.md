...
Non-Destructive methods of manipulating arrays allow you to make copies of the original array that can be changed while still keeping the original array intact. 
...
```javascript
const pokemon = [`Bulbasaur`, `Charmander`, `Squirtle`, `Pikachu`, `Geodude`]; const slicedPokemon = pokemon.slice(2); console.log(slicedPokemon); // Output: ["Squirtle", "Pikachu", "Geodude"]
```
