...
`*(max-min+1)+min)` multiplies the random number generated by the value of `max - min+1`. It has to be plus 1 because we're dealing with index values, and JavaScript uses a zero index. It then adds the value of `min`. This means that the value of `min` is included. While min is zero at the moment, the value of min might change in the future, and this would need to be debugged without ``
