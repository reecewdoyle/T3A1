...
A `JSON` ( JavaScript Object Notation) looks very similar to an object. The primary difference is that a JSON is mostly used for data-interchange in API's and for configuration settings. An `object` is used in programming, and can therefore be found in multiple programming languages, and is more likely to contain functions. 

While it's possible to have functions in a `JSON`, it is less likely to, as they're more often used to store data in key-value pairs. 

#### Parsing JSON

Parsing is the process of analysing a JSON like string and creating a JSON that can be manipulated and and accessed through your programming language; in this case it's JavaScript.
```javascript

const jsonString = `{"name": "Tony Stark", "alias": "Iron Man`
```
