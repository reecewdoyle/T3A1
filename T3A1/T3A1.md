...
"mod" is the parameter being passed by the constructor.
*/
	} 
	show() { 
...
```javascript
	...
	mycar = new Model(make, model); 
/*
`mycar` is declared as a variable.
`new` creates a new instance of the `Model` class from above. `make` and `model` are passed as arguments. This allows access to the code above that randomly selects a `make` from the array of ["Ford", "Holden", "Toyota"], and a year from the `model` array that generates every number between 1980 and 2019.

*/
	console.log(mycar.show()) 
// this prints the `mycar` to the console in the browser. 
}
```
***