...
Creating this app required me to acquire skills in Python, but also in multiple Python packages. 
...
#### Date Time
On the functions page, I used Python's built in `datetime` package to solve a problem I was having. I wanted to create a function that printed only the games that were occurring in that particular week. In order to do this, I realised that I would have to use the `week_num` function. 
...
Weeks of the year solved a use case. I wanted the user to be able to look on a Tuesday what games were being played on the weekend. As I'd already defined the games by game numbers and dates, the most obvious way to sort by the week number, which is called `week_num` in `datetime`. This would allow me to see what games were occurring that week, as several games would have the same `week_num`. 
...
I stored the draw for the competition in a seperate csv file called `lmpl.csv`. This meant I had to add an extra column to my `lmpl.csv`, but it was well worth it:
...
I once again used the `csv reader` to look at the `lmpl.csv`. 
